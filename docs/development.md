...
The main page and Streamer builder include a Buy Me a Coffee support link. Set `supportUrls.buymeacoffee` in [support-config.js](../controller/support-config.js) to a verified creator-page URL. The support section stays hidden unless a valid Buy Me a Coffee HTTPS URL is configured. The capture overlay contains no support UI. Tips are handled on Buy Me a Coffee, and existing app features remain free.
