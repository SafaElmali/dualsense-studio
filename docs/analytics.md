...
Streamer gyro uses `controller_streamer_settings_changed` with `setting=gyro` and `gyro=off|subtle|full`, plus `surface=builder|capture`. Choosing Recenter records `controller_streamer_gyro_recentered` with the capture surface only. Neither event includes motion samples, neutral orientation, calibration data, or device identifiers.
