...
Choose **Controller motion → Subtle** or **Full rotation** in the editor's Camera tab, or **Follow controller motion** in capture setup. Gyro uses a direct DualSense connection in a desktop Chrome or Edge capture window. Select **Connect DualSense**, hold the controller comfortably, and choose **Recenter motion**. In OBS, use Window Capture and a green or blue Chroma Key background. Keep the controller tab visible and the window unminimized; the game can have focus. Normal OBS Browser Source input does not supply gyro data.

**Subtle** adds 35% of the physical rotation around your chosen camera angle, capped at 25° so buttons stay readable. **Full rotation** follows relative turns at their full size and adds framing room for the rotating controller. Both use the view's frame-rate-independent quaternion smoothing. Auto camera following pauses while gyro is connected; its resting angle is the Angle view. Dragging the model turns gyro off. Turning gyro off restores your selected camera behavior.

The first motion sample establishes a neutral holding position. **Recenter motion** immediately makes the current holding position match your chosen camera angle again; it does not run the playground's stationary bias calibration. Gravity limits tilt drift, but heading can still drift, so recenter when needed. Missing reports and disconnects clear the neutral reference; resumed reports establish a fresh position without replaying missed motion. Buttons, sticks, triggers and finger tracking continue alongside gyro.

Motion mode is saved in copied links and local saved looks; old links default to Off. Opening a link never launches a permission prompt. A single previously authorized controller can reconnect automatically. Motion samples and the neutral pose stay in the browser and are never saved in links, presets or analytics. The shared input connection only reads feature report 0x05 for factory sensitivity and extended Bluetooth reports; no output or calibration writes are added. USB is the simplest starting point.

Automated checks cover USB/Bluetooth sensor reports reaching the overlay, relative axes and chosen camera angles, subtle rotation limits, recentering, stale input, disconnect/reconnect, cancellation during calibration, link defaults and analytics boundaries. Physical direction, drift, smoothing feel and OBS capture while gaming still require a hands-on controller check.
