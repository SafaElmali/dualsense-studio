...
- Drag the shell to rotate the controller; hold Alt (Option on Mac) and scroll to zoom. Ordinary scrolling moves the page. Dragging or choosing a fixed angle pauses Auto. Choose Auto to resume, or Reset to restore the front and Auto. Camera movement respects reduced-motion preferences.
...
Open **Controller diagnostics** for three tools:

- **Stick lab** shows raw browser-reported stick axes, or direct DualSense input reports when connected through the panel. **Measure resting sticks** samples for two seconds and reports mean and peak offset. **Start range sweep** collects the farthest point reached in each of 48 angular sectors. Circularity error appears only after every sector has a sample; it is RMS deviation from a unit circle, not a health grade. A lower percentage is not automatically better. Full-range symmetry matters too.
- **Guided checkup** walks through reported buttons, sticks, trigger travel, touchpad movement/two-finger tracking/click, three motion axes, adaptive trigger resistance, lightbar colors, left/right vibration, and the speaker. Observed input checks are labeled **Observed**. Effects need an explicit **Start test**, followed by the user's **Worked for me** or **I noticed an issue**. A successful write is never treated as a physical pass. Any check can be skipped. **Download checkup card** creates a local 1200 × 900 PNG with measurements and clearly labeled observed, confirmed, issue, skipped, and untested results; unfinished sessions are marked partial.
- **Controller details** reads the model, board revision, firmware version/build date, and original finish when available. Finish lookup uses USB and is unavailable for Edge or unknown serial codes. **Match the 3D controller** applies a supported factory finish to the virtual model only. Other solid colors are approximate; special-edition artwork and replacement shells are not inferred. Serial numbers are discarded after lookup, never displayed, stored, sent to analytics, or included on the card.

A standard gamepad works for sticks, button presses/releases, and L2/R2 travel. **Home / PS** is optional in the button check because the operating system may intercept it to open another app, as on macOS. Every other reported button still requires a press and release. Home stays untested until observed, and the downloaded card identifies whether it was tested. **Connect DualSense** grants optional WebHID access in desktop Chrome or Edge. The selected HID controller supplies all diagnostic inputs so readings from separate controllers cannot mix. The speaker tone requires USB and no headphones. Motion bars use nominal sensor sensitivity and are labeled approximate. Receiving inputs verifies response, not repair status or end-to-end latency.

Opening diagnostics releases existing trigger effects. Vibration runs briefly in the left grip then right; resistance lasts three seconds, lightbar colors cycle for 1.5 seconds and restore the selected color, and the speaker tone lasts 0.7 seconds. Effects use the shared output queue and stop on tab/step changes, focus loss, panel close, or page exit. Closing the page attempts cleanup before releasing the shared connection; a browser crash or failed hardware write may require unplugging the controller. Reconnect, reset, or reopening starts a new checkup. Focus loss cancels an incomplete resting measurement and stops range collection. No stick calibration or persistent-memory commands are sent.

Unit coverage includes sweep completeness, peak retention, checkup evidence, report framing, firmware/finish decoding, serial privacy, timeouts, USB/Bluetooth output isolation, test timing, cancellation during writes, and shutdown cleanup. Browser checks use synthetic reports; physical feel, light colors, and speaker output still require hands-on verification. Adapted code and full MIT credit are documented in [DIAGNOSTICS-NOTICES.md](../controller/DIAGNOSTICS-NOTICES.md).
