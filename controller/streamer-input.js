...
// Read-only capture input. Never changes trigger effects, rumble, LEDs or calibration.
// Raw packet fields follow Sony's Linux hid-playstation driver (TRIGGER-NOTICES.md).
export class StreamerInput {
  ...
  constructor(input, { hid, getGamepads = () => [], onStatus = () => {}, onTouch = () => {}, now = () => performance.now() } = {}) {
    ...
    this.onReport = event => {
      ...
      const frame = StreamerInput.decode(event.reportId, event.data);
      if (!frame) return;
  ...
  apply({ buttons, axes }) {
    StreamerInput.buttons.forEach((id, index) => this.input.setButton(id, 'gamepad', buttons[index] || 0));
  ...
  poll() {
    ...
    if (this.device) {
      if (this.now() - this.lastReport > 1500) { this.clear(); this.status('waiting-direct'); }
      return;
    }
    let pads;
  ...
  async connect({ automatic = false } = {}) {
    ...
    let device;
    try {
      const devices = automatic ? (await this.hid.getDevices()).filter(device => StreamerInput.supported(device)) : await this.hid.requestDevice({ filters: StreamerInput.filters });
      if (generation !== this.generation || devices.length !== 1 || !StreamerInput.supported(devices[0])) return false;
      device = devices[0];
      await device.open();
      ...
      try { await device.receiveFeatureReport(0x05); } catch { /* USB needs no activation; Bluetooth can fall back to USB. */ }
